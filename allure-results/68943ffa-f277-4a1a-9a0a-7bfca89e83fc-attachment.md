...
```
Error: locator.waitFor: Error: strict mode violation: locator('text=Stock Items').filter({ hasText: 'Stock Items' }) resolved to 2 elements:
...
Call log:
  - waiting for locator('text=Stock Items').filter({ hasText: 'Stock Items' }) to be visible
